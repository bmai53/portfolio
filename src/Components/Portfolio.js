  ...
  render() {
    ...
    if (this.props.data) {
      var projects = this.props.data.projects.map(function (projects, index) {
        ...
        return <div key={projects.title} className="columns portfolio-item" style={
          index === 3 ? { clear: 'both', float: 'left', display: 'block' } : {}
        }>
            ...
            <a href={projects.url} title={projects.title} target="_blank">
              ...
              <div className="overlay">
                <div className="portfolio-item-meta">
                  <h5 style={{ position: 'absolute', top: '15px', left:"15px", fontSize: '30px' }}>{projects.title}</h5>
                  <div style={{ position: 'absolute', bottom: '15px', right: '15px'}}>
                    {
                      projects.category.map(pc => <p style={{ textAlign: 'right', fontSize: '20px', margin: '5px' }}>{pc}</p>)
                    }
                  </div>
                </div>
              </div>
